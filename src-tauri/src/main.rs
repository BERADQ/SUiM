...
async fn image_base64(path: String) -> Result<String, String> {
    ...
    if let Ok(_) = file.read_to_end(&mut file_content) {
      return if let Ok(image) = image::load_from_memory(&file_content) {
        ...
        if image.height() > MAX_SIZE || image.width() > MAX_SIZE {
          ...
          resized.write_to(&mut compress_content, image::ImageOutputFormat::Jpeg(80)).expect("ERR:COMPRESS_IMAGE");
          encoded = general_purpose::STANDARD_NO_PAD.encode(compress_content.into_inner());
        } else {
  ...
  } else { return Err("ERR:OPEN_FILE".to_string()); }
